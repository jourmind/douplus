...
- 订单续费（单个/批量）
- 批量续费（`POST /api/douplus/order/batch-renew`）
- 业务规则验证
- **原则**: 轻量级业务逻辑
...
**接口**: `POST /api/douplus/order/task/renew`

**用途**: 对单个投放中的订单追加预算和延长时长

**请求参数**:
```json
{
  "orderId": "7459361994885537055",  // 订单ID
  "budget": 100,                      // 追加预算（元）
  "duration": 24,                     // 延长时长（小时，可为0）
  "investPassword": "123456"          // 投放密码（可选）
}
```

**业务规则**:
- budget必须大于0（不可以仅增加时长）
- duration可以为0（可以仅追加预算）
- 只能对投放中（DELIVERING/RUNNING）的订单续费

**响应**:
```json
{
  "code": 200,
  "message": "续费成功！已追加100元预算，延长24小时",
  "success": true
}
```
...
**接口**: `POST /api/douplus/order/batch-renew`

**用途**: 对多个投放中的订单批量续费

**使用场景**: 在"我的订单"页面（DOU+投放记录页面）选择多个投放中的订单，统一设置追加预算和延长时长，一次性完成续费操作
...
```json
{
  ...
  "budget": 100,                       // 每个订单追加的预算（元）
  "duration": 24,                      // 每个订单延长的时长（小时，可为0）
  "investPassword": "123456"           // 投放密码（可选）
}
```
...
```json
{
  ...
  "data": {
    ...
    "details": [
      {
        "orderId": "7459361994885537055",
        "success": true,
        "message": "续费成功！已追加100元预算，延长24小时"
      },
      {
        "orderId": "7459362001234567890",
        "success": true,
        "message": "续费成功！已追加100元预算，延长24小时"
      }
    ]
...
**前端操作流程**:
1. 进入"我的订单"页面（`/douplus-history`）
2. 左侧多选框选择多个投放中的订单（非投放中的订单不可选）
3. 点击"批量续费"按钮
4. 在弹窗中设置统一的追加金额和延长时长
5. 显示预计总消耗（订单数 × 追加金额）
6. 提交后，后端逐个处理并返回详细结果
7. 前端显示续费结果摘要，自动刷新订单列表
