...
# 订单信息（使用正确的task_id）
order_id = '1855431292118059'
task_id = '1855431292118043'  # ⚠️ 这是真正需要的task_id！
budget = 100  # 元
duration = 12  # 小时（改用12小时测试）
...
try:
    ...
    internal_id, db_order_id, account_id, status, item_id, aweme_sec_uid, open_id, token_encrypted = order
    
    print(f"✅ 订单查询成功:")
    print(f"   - 内部ID: {internal_id}")
    print(f"   - 数据库order_id: {db_order_id}")
    print(f"   - 使用task_id: {task_id} ⚠️ (续费API需要)")
